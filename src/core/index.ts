...
export function init() {
  ...
  camera = new THREE.PerspectiveCamera(40, window.innerWidth / window.innerHeight, 1, 1000); // 创建主相机
  ...
  const points = GeometryUtils.hilbert3D(new THREE.Vector3(0, 0, 0), 20.0, 1, 0, 1, 2, 3, 4, 5, 6, 7); // 生成 Hilbert 曲线点
  ...
  matLine = new LineMaterial({
    ...
    dashed: false, // 是否虚线
    alphaToCoverage: true, // 是否启用 alphaToCoverage
  });
  ...
  // THREE.Line (THREE.BufferGeometry, THREE.LineBasicMaterial) - 使用 gl.LINE_STRIP 渲染
  ...
  matLineDashed = new THREE.LineDashedMaterial({ vertexColors: true, scale: 2, dashSize: 1, gapSize: 1 }); // 创建虚线材质
  ...
  document.body.appendChild(stats.dom); // 将性能监控的 DOM 元素添加到文档中
